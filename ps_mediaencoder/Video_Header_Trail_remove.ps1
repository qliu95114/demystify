...
If ((Test-Path $filename) -and (Test-Path $outputfolder))
{
    ...
    if ($null -eq $crf)
    {
        $ffcmd="ffmpeg.exe -y -i ""$($filename)"" -ss $($starttime).000 -to $($endtime).000  -c:v $($gpu) -crf $($crf) -preset slow -pix_fmt nv12 -vf ""scale=1920:-2"" -map 0:v:0? -map 0:a:0? -c:a aac -b:a $($bitrate_audio)k -map -0:s -map_chapters 0 -map_metadata 0 -f mp4 -threads 0 ""$($outputfile)"" 2> ""$($logfile)"""
    }
    else {
        $ffcmd="ffmpeg.exe -y -i ""$($filename)"" -ss $($starttime).000 -to $($endtime).000  -c:v $($gpu) -b:v $($bitrate)k -pix_fmt nv12 -vf ""scale=1920:-2"" -map 0:v:0? -map 0:a:0? -c:a aac -b:a $($bitrate_audio)k -map -0:s -map_chapters 0 -map_metadata 0 -f mp4 -threads 0 ""$($outputfile)"" 2> ""$($logfile)"""
    }
